...
// Test DM messaging functionality with the fixed user-online event
async function testDMFixedMessages() {
  console.log('Testing DM message functionality with user-online fix...\n');
  ...
  // Emit user-online events (this is the fix!)
  console.log('\n4. Emitting user-online events...');
  ...
  setTimeout(() => {
    const msg = {
      from: semenUserId,
      to: trexUserId,
      content: 'Hello from Semen with fixed user-online!',
      timestamp: Date.now(),
      room: roomId,
    };
  ...
  setTimeout(async () => {
    ...
    console.log('\n✅ Test completed - DM functionality should now work!');
    process.exit(0);
  }, 3000);
}

testDMFixedMessages().catch(err => {
