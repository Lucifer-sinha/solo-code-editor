...
const findNodeByPath = (fileSystem: FileSystemNode, path: string): FileSystemNode | null => {
...
export const useFileSystem = (): FileSystemHook => {
  ...
  const saveFile = (path: string, content: string): boolean => {
    ...
    const newFileSystem = JSON.parse(JSON.stringify(fileSystem)) as FileSystemNode;
  ...
  const renameItem = (path: string, newName: string): boolean => {
    if (!fileSystem) return false;
  ...
  const exportAsZip = async (): Promise<boolean> => {
    try {
